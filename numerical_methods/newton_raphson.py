...
def newton_raphson(exp, xi, tolerance , mx_iterations,top,pre):
    ...
    mylist.insert(END,"Newton Raphson")
    while (error > tolerance and i <= mx_iterations):
        mylist.insert(END,"Iteration " + str(i)+"\n xi: "+str(xi))
        ...
        if (df_subs == 0):
            mylist.insert(END,"horizontal tangent and no root")
            #print("horizontal tangent and no root")
            messagebox.showerror("invalid value", "horizontal tangent and no root")
            top.destroy()
            return
        ...
        if (i > 1):
            ...
            mylist.insert(END,"Approximation error: " + str(error) + "\n")
