...
def Falsi(exp, x, xl, xu, top,pre,es=1e-5, maxIterations=50):
    ...
    if fun(xl, exp) < 0 and fun(xu, exp) > 0:
        ...
        messagebox.showerror("invalid value", 'No root can be found!')
        top.destroy()
        return
    ...
    mylist.insert(END,"The False-Position Method:")
    while ea >= es and numGuesses <= maxIterations:
        mylist.insert(END,'%.2d. ,xl = %.6f\t,xu = %.6f\t,ans = %.6f\t,Ea = %.6f%%' % (numGuesses, xl, xu, ans, ea*100))
